...
In this case it returns a vector containing the first three values in the RBC column and all the values in the WBC column because these are all less than 10.
...
Or we can save the step of assigning an intermediate variable by instead using the logical expression directly when indexing the matrix:
...
When executing a logical expression with a **matrix column**, it returns a **vector of logical values**. Each element in the matrix column is evaluated individually.
...
For the rest of this section we will work with expression levels for 7 randomly chosen genes across 5 human tissues (whole blood, liver, lung, pancreas, and stomach). Expression levels are measured in transcripts per million (TPM), where the higher the TPM value, the higher the gene expression in that tissue:
...
The correlation between the expression of the 7 genes in blood and liver is 0.41, as seen at both `[blood, liver]` and `[liver, blood]`. 

The correlation between the expression of the 7 genes in pancreas and stomach is 0.42, as seen at both `[pancreas, stomach]` and `[stomach, pancreas]`. 

This is a symmetric 2D matrix, as discussed in 4.1.1. The rows and columns are the same set of samples, and each value is a measurement of the relationship between the samples.
...
An R package is a collection of functions and/or data sets that can be installed to add greater functionality to R. To the right you will see a tab called "Packages" that lists all of the packages already installed to the JupyterHub R server.
...
Many R packages, including `pheatmap`, can be installed using the `install.packages()` function:
...
The dendrogram to the left of the plot shows the clustering of the 5 different tissues based on the similarity of their expression profiles for the 7 genes. 
