...
In a symmetric matrix, the value at row x, column y is the same as the value at column y, row x. For example, P2 and PN interact, so there is a `1` at row P2, column PN and there is a `1` at row PN, column P2.
...
Looking at the first row of the table of data and the first row of the matrix we can see that they do not match. The values of the first row in the table are in column one and the top of column two. This is because the matrix was filled by column by default.
...
The resulting matrix will automatically have row names, which are the names of each vector.
...
Alternatively, the function `dim()` returns a vector of the number of rows followed by the number of columns:
...
You can access multiple rows or multiple columns by putting a vector of indices in place of the row or column index:
        ...
        matrix_name[row_indices,]
        matrix_name[,column_indices]
...
You can also access one or more rows AND one or more columns:
        ...
        matrix_name[row_indices,column_indices]
