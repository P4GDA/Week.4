...
We will start by looking a few randomly chosen SNPs across 4 populations:
- African (AFR)
- East Asian (EA)
- European (EUR)
- South Asian (SA)

SNPs have ids in the form "rsXXXX" where the Xs are a number. Here we will look at the SNPs: rs671, rs4680, and rs6265.

These are the frequencies of the *alternative allele* in each of the 4 populations:
...
The following vector contains the value in the table above, going from left to right along each row in the table.
...
Using named indexing, assign the alternative allele frequency for rs671 and rs6265 in the EA and EUR populations to a variable named "SNPs_subset".
```{r}
